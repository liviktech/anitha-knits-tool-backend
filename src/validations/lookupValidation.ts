...
// Mirrors expenseValidation.ts's expenseName max length, so every seeded/admin-added
// suggestion is guaranteed to fit the field it ultimately populates.
const expenseNameNameSchema = z.string().trim().min(1, 'name is required').max(150);
...
export const createExpenseNameSchema = z.object({ name: expenseNameNameSchema }).strict();
...
export const updateExpenseNameSchema = z.object({ name: expenseNameNameSchema }).strict();
...
export type CreateExpenseNameInput = z.infer<typeof createExpenseNameSchema>;
...
export type UpdateExpenseNameInput = z.infer<typeof updateExpenseNameSchema>;
